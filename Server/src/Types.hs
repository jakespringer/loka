...
------------------------------------------------------------------------------
data Roll = Roll Integer deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
...
data PieceDirection = DirectionRight | DirectionUpRight | DirectionUp
  ...
  | DirectionDownRight deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
data KnightDirection = KnightDirectionUpRight | KnightDirectionUpLeft
  ...
  | KnightDirectionDownRight | KnightDirectionDownLeft deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
data MountainPassDirection = MountainPassDirectionHorizontal
  | MountainPassDirectionVertical deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)

------------------------------------------------------------------------------
data PieceType = King | Queen | Rook | Bishop | Knight | Pawn
  deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
data PieceColor = Red | Green | Blue | Yellow deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
data PieceMove = KingMove { direction :: PieceDirection }
  ...
  | PawnMove deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
data Piece = Piece { pieceX :: Integer
  , pieceY :: Integer
  , pieceType :: PieceType
  , color :: PieceColor } deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)

------------------------------------------------------------------------------
data TerrainType = Eyrie | Castle | Swamp | MountainPass MountainPassDirection
  | Forest | Lake | StoneCircle | Portal deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
data Terrain = Terrain { terrainX :: Integer
  , terrainY :: Integer
  , terrainType :: TerrainType } deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
...
data GameMove = Noop
  ...
  | AttackPiece { move :: PieceMove
    ...
    , attackerRoll :: Roll
    , defenderRoll :: Roll }
  | MultiMove [GameMove] deriving (Show, Read, Eq, Generic, ToJSON, FromJSON)
